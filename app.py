import streamlit as stimport pandas as pdimport plotly.express as pxfrom sklearn.ensemble import RandomForestClassifierfrom sklearn.model_selection import train_test_splitimport base64
...
df['delay'] = pd.to_numeric(df.get('delay'), errors='coerce')df['attempts'] = pd.to_numeric(df.get('attempts'), errors='coerce')

df['risk'] = df.get('risk').fillna("Unknown")df['payment_type'] = df.get('payment_type').fillna("Unknown")df['status'] = df.get('status').fillna("Unknown")
...
def get_base64(bin_file):with open(bin_file, 'rb') as f:data = f.read()return base64.b64encode(data).decode()

try:logo_base64 = get_base64("logo.png")except:logo_base64 = ""
...
payment_options = sorted(df['payment_type'].unique())risk_options = sorted(df['risk'].unique())
...
if payment:df = df[df['payment_type'].isin(payment)]

if risk:df = df[df['risk'].isin(risk)]

if df.empty:st.warning("No data available for selected filters")st.stop()
